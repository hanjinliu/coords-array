...
from ._index import as_index, Index, ScaledIndex, IndexLike
...
@overload
def pick_axis(shape: int, exclude: Sequence[str] = ()) -> Axis:
    ...


@overload
def pick_axis(shape: tuple[int, ...], exclude: Sequence[str] = ()) -> list[Axis]:
    ...


def pick_axis(shape, exclude=()):
    """Pick axes with given shape."""
    i = j = 0
    is_scalar = isinstance(shape, int)
    if is_scalar:
        shape = (shape,)
    jmax = len(shape)
    out: list[Axis] = []
    while True:
        ...
        if name not in exclude:
            out.append(Axis.from_size(name, shape[j]))
            j += 1
            if jmax == j:
                break
        i += 1
...
class LinearTransformedAxis(_AxisBase):
    ...
    def __init__(self, name: str, components: dict[_AxisBase, float]):
        ...
        base_units = {axis.unit for axis in components.keys()}
        if len(base_units) == 1:
    ...
    def from_linear_combination(
        ...
        if name is None:
            name = "".join(
                f"{k:+.2g}{axis}" for axis, k in axis_to_coef.items()
            ).lstrip("+")
